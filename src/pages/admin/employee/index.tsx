...
export default function CardSettings() {
  ...
  const { data: EmployeeData } = api.users.getAll.useQuery();
            ...
            <table className="w-full border-collapse items-center bg-transparent">
              ...
              <tbody>
                {EmployeeData?.map((employee) => (
                  <TableRows
                    key={employee.id}
                    name={employee.name || "no user"}
                    email={employee.email || "no user"}
                  />
                ))}
              </tbody>
            </table>
...
const TableRows = ({ name, email }: { name: string; email: string }) => {
  return (
    <tr>
